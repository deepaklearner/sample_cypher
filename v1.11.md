...
I want to add a check in below cypher query if managerid is numeric then run this cypher "OPTIONAL MATCH (m:User {employeeNumber: e.managerid})" or if managerid is non-numeric then run this cypher "OPTIONAL MATCH (m:User {aetnaresourceid: e.managerid})"

Note: 
1. Ensure there are no duplicates in return.
2. Use COLLECT to collect the managers.
3. write a optimized cypher query.
